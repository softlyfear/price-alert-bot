...
- Исполнитель: цепочка `<слоевой-агент>` → … → `tests-agent` — вертикальный срез: законченная функция через все нужные слои, исполнители по очереди в порядке зависимостей слоёв, каждый только в своей зоне, одно ревью, одна приёмка, один коммит (соглашение 9). Больше ~4 исполнителей — срез режется на два. Один агент — только когда функция целиком лежит в одной зоне. Агенты: `db-models-agent`, `schemas-agent`, `repository-agent`, `integrations-agent`, `service-agent`, `bot-agent`, `core-agent`, `tests-agent`, `devops-agent`.
